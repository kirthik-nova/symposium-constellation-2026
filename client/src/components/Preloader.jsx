...
const Preloader = ({ onComplete }) => {
  ...
  const handleGlobalClick = (e) => {
    ...
    // Clear all existing stage-transition timers
    timersRef.current.forEach(clearTimeout);
    timersRef.current = [];

    if (stage < 4) {
      // Advance to the cinematic text screen
      setStage(4);
      setVisibleWords(5); // Show all words immediately if skipped to this stage
    } else if (stage === 4) {
      // Already at text screen, proceed to home
      onComplete();
    }
  };
  ...
  return (
    <div onClick={handleGlobalClick} className="cursor-pointer">
      <AnimatePresence>
